...
class ModelConfig(Base, BaseModelMixin):
    ...
    currency = Column(String(10), default="CNY", comment="价格币种: CNY/USD/EUR/JPY/KRW等")
    input_price = Column(Integer, default=0, comment="输入价格(每百万token)，单位分/0.0001美元等整数缩放")
    output_price = Column(Integer, default=0, comment="输出价格(每百万token)，单位分/0.0001美元等整数缩放")
    price_unit = Column(String(20), default="1M_tokens", comment="价格单位: 1M_tokens")
