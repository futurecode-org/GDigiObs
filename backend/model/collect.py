...
class CollectPlatform(Base, BaseModelMixin):
    ...
    default_method = Column(String(20), default="api", comment="默认采集方式: api/rss/crawler")
...
class CollectTask(Base, BaseModelMixin):
    ...
    platform_id = Column(Integer, ForeignKey("collect_platforms.id"), nullable=False, comment="平台ID")
    collect_method = Column(String(20), nullable=False, comment="采集方式: api/rss/crawler")
    source_url = Column(Text, nullable=True, comment="采集地址")
    request_config = Column(JSON, nullable=True, comment="请求配置")
    parse_rule = Column(JSON, nullable=True, comment="解析规则")
    schedule_config = Column(JSON, nullable=True, comment="定时配置")
    is_public = Column(Boolean, default=False, comment="是否公开")
    status = Column(String(20), default="draft", comment="状态: draft/enabled/disabled/error")
...
class CollectedItem(Base, BaseModelMixin):
    ...
    tenant_id = Column(Integer, ForeignKey("tenants.id"), nullable=False, index=True, comment="租户ID")
    task_id = Column(Integer, ForeignKey("collect_tasks.id"), nullable=False, index=True, comment="任务ID")
    ...
    raw_content_type = Column(String(50), default="html", comment="原始内容类型: html/json/text")
    content_hash = Column(String(128), nullable=True, index=True, comment="内容Hash")
    sentiment = Column(String(20), nullable=True, comment="情感: positive/neutral/negative")
    tags = Column(JSON, nullable=True, comment="标签")
    entities = Column(JSON, nullable=True, comment="实体")
    is_public = Column(Boolean, default=False, comment="是否公开")
    status = Column(String(20), default="raw", comment="状态: raw/cleaned/analyzed/duplicate/error")
